...
window.TK.DateTime = {
    ...
    RefreshDateInput: function (dontFocus) {
        ...
        // class toggle with support for older browsers
        var safeToggle = function (list, name, newValue) {
            if (newValue && !list.contains(name))
                list.add(name);
            else if (!newValue && list.contains(name))
                list.remove(name);
        };
        

        safeToggle(this.classList, "toolkitDateTimeDisabled", this.disabled);
        safeToggle(this.classList, "toolkitDateTimeReadOnly", this.readOnly);
        safeToggle(this.classList, "toolkitDateTimeRelative", isRelative);
